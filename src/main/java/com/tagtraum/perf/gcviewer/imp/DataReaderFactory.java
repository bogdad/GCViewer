...
public class DataReaderFactory {
    ...
    static final int THIRTY_TWO_KB = ONE_KB * 32;
    ...
    public DataReader getDataReader(GCResource gcResource, InputStream inStream) throws IOException {
        ...
        InputStream in = new BufferedInputStream(inStream, THIRTY_TWO_KB);
        // isGZipped relies on streams to support "mark" -> BufferdInputStream does
        if (isGZipped(in)) {
            getLogger().info("GZip stream detected");
            in = new BufferedInputStream(new GZIPInputStream(in, THIRTY_TWO_KB), THIRTY_TWO_KB);
        }
        ...
        while (attemptCount < MAX_ATTEMPT_COUNT) {
            in.mark(THIRTY_TWO_KB + (int) nextPos);
            if (nextPos > 0) {
